...
TopoBot.prototype._log = function ( message ) {
    if ( DEBUG_MODE ) {
        this.postMessage( DEBUG_CHANNEL , message , { as_user : true } );
    }
}
    ...
    var checkPhone = setInterval( function( phoneip ) {
        ping.sys.probe( phoneip , function ( isAlive ) {
            ...
            if ( ( date_difference > away_time ) && ( !away ) ) {
                away = true;
                self._setAllOff();
                this._log( 'Phone went away at: ' + new Date() );
            }
            if ( ( date_difference < away_time ) && ( away ) ){
                away = false;
                self._setAllLightsOn();
                this._log( 'Phone came back at: ' + new Date () );
            }
        });
...
TopoBot.prototype._onMessage = function ( message ) {
    ...
    if ( this._isMentioningTopoBot( message ) &&
         this._isKillerSignal( message ) ) {
            this._log( 'Terminating at: ' + new Date() );
            process.exit( 1 );
    }
};
